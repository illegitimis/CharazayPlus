...
namespace AndreiPopescu.CharazayPlus.UI
{
    ...
    // Pixel-Position of mark buttons
    private int PosL=0,PosR=0;	
    ...
    #region user control event handlers overrides
    ...
    private void OnPaint (object sender, System.Windows.Forms.PaintEventArgs e)
    {
      ...
      if (this.orientationBar == RangeBarOrientation.horizontal)
        PaintHorizontalOrientation(e, h, w, penShadowLight, penShadowDark, brushShadowLight, brushShadowDark, brushInner, brushRange, out baryoff, out markyoff, out tickyoff1, out tickyoff2, out dtick, out tickpos);
      else // vertical bar
        PaintVerticalOrientation(e, h, w, penShadowLight, penShadowDark, brushShadowLight, brushShadowDark, brushInner, brushRange, out baryoff, out markyoff, out tickyoff1, out tickyoff2, out dtick, out tickpos);

    }

    private void PaintVerticalOrientation (System.Windows.Forms.PaintEventArgs e, int h, int w, Pen penShadowLight, Pen penShadowDark, SolidBrush brushShadowLight, SolidBrush brushShadowDark, SolidBrush brushInner, SolidBrush brushRange
      , out int baryoff, out int markyoff, out int tickyoff1, out int tickyoff2, out double dtick, out int tickpos)
    {
      dtick = 0d;
      tickpos = 0;
    ...
    private void PaintHorizontalOrientation (System.Windows.Forms.PaintEventArgs e, int h, int w, Pen penShadowLight, Pen penShadowDark, SolidBrush brushShadowLight, SolidBrush brushShadowDark, SolidBrush brushInner, SolidBrush brushRange
      , out int baryoff, out int markyoff, out int tickyoff1, out int tickyoff2, out double dtick, out int tickpos)
    {
      dtick = 0d;
      tickpos = 0;
      ...
      {
        e.Graphics.DrawString(String.Format("[{0}-{1}]",rangeMin,rangeMax), new Font("Arial", MarkWidth), new SolidBrush(colorShadowDark)
          , w / 2, h-tickyoff1 - TickHeight-markyoff, new StringFormat() { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Near });
      }
    }
